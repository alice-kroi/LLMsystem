...
class DoubaoEmbeddings(Embeddings):
    def __init__(self, api_key, model="doubao-embedding-text-240715"):
        self.client = OpenAI(
...
class RAG:
    def __init__(self, modeltype="doubao"):
        """
        初始化RAG类，加载配置并设置默认参数
        
        Args:
            modeltype (str): 模型类型，可选值为"doubao"或"zhipu"，默认为"doubao"
        """
        logger.info(f"初始化RAG类，模型类型: {modeltype}...")
        ...
        if self.modeltype == "doubao":
            # 豆包模型配置
            self.api_key = os.environ.get('ARK_API_KEY')
            if not self.api_key:
                raise ValueError("未找到ARK_API_KEY环境变量，请配置豆包API密钥")
            self.embedding_model = "doubao-embedding-text-240715"
        elif self.modeltype == "zhipu":
            # 智谱模型配置
            self.api_key = os.environ.get('API_KEY')
            if not self.api_key:
                raise ValueError("未找到API_KEY环境变量，请配置智谱AI API密钥")
            self.embedding_model = "embedding-3"
        else:
            raise ValueError(f"不支持的模型类型: {modeltype}，可选值为'doubao'或'zhipu'")
    ...
    def create_vectorstore(self, vectorstore_path=None, embedding_model=None):
        ...
        logger.info(f"准备创建向量库，路径: {path}, 嵌入模型: {model}, 模型类型: {self.modeltype}")
        ...
        try:
            ...
            # 根据模型类型初始化相应的嵌入模型
            if self.modeltype == "doubao":
                logger.info(f"初始化豆包嵌入模型: {model}")
                self.embeddings = DoubaoEmbeddings(api_key=self.api_key, model=model)
            else:
    ...
    def get_vectorstore(self):
        ...
        try:
            ...
            if self.embeddings is None:
                if self.modeltype == "doubao":
                    logger.info(f"初始化豆包嵌入模型: {self.embedding_model}")
                    self.embeddings = DoubaoEmbeddings(api_key=self.api_key, model=self.embedding_model)
                else:
    ...
    def retrieve(self, query, top_k=3):
        ...
        Args:
            query (str): 查询文本
            top_k (int, optional): 返回的相关文档数量，默认为3
        ...
        try:
            ...
            if self.vectorstore is None:
                logger.info("向量库未加载，尝试自动加载")
                self.get_vectorstore()
                if self.vectorstore is None:
                    logger.error("无法加载向量库")
                    return []
            
            # 执行相似性搜索
            logger.info(f"执行向量库搜索，查询: {query[:50]}..., top_k: {top_k}")
            ...
            logger.error(f"检索相关信息时出错: {str(e)}")
            return []
...
if __name__ == "__main__":
    try:
        # 测试1: 使用豆包模型（默认）
        print("=== 测试1: 使用豆包模型 ===")
        rag_doubao = RAG()  # 默认使用豆包模型
        
        # 检查向量库是否存在
        exists = rag_doubao.check_vectorstore_exists()
        print(f"向量库存在: {exists}")
        
        # 如果不存在则创建
        if not exists:
            created = rag_doubao.create_vectorstore()
            print(f"向量库创建: {created}")
        
        # 加载向量库
        vectorstore = rag_doubao.get_vectorstore()
        print(f"向量库加载: {vectorstore is not None}")
        
        # 创建测试文档
        test_content = "这是一个测试文档，用于测试豆包模型的RAG功能。"
        test_file_path = os.path.join(rag_doubao.documents_path, "test_doubao.txt")
        ...
        # 添加文档到向量库
        add_success = rag_doubao.add_document_to_vectorstore(test_file_path)
        print(f"测试文件添加: {add_success}")
        
        # 搜索知识库
        query = "豆包模型的RAG功能"
        print(f"\n搜索查询: {query}")
        results = rag_doubao.search_knowledge_base(query, k=3)
        
        if results and results['results']:
            print(f"找到{len(results['results'])}个相关信息：")
            for i, result in enumerate(results['results'], 1):
                content = result['content'][:200] + ("..." if len(result['content']) > 200 else "")
            ...
            if results['sources']:
                print(f"来源文件: {', '.join(results['sources'])}")
        else:
            print("未找到相关信息")
        ...
        # 测试2: 使用智谱模型
        print("\n\n=== 测试2: 使用智谱模型 ===")
        rag_zhipu = RAG(modeltype="zhipu")
        
        # 检查向量库是否存在
        exists = rag_zhipu.check_vectorstore_exists()
        print(f"向量库存在: {exists}")
        
        # 如果不存在则创建
        if not exists:
            created = rag_zhipu.create_vectorstore()
            print(f"向量库创建: {created}")
        
        # 加载向量库
        vectorstore = rag_zhipu.get_vectorstore()
        print(f"向量库加载: {vectorstore is not None}")
